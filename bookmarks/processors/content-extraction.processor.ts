 ...
 */
class ContentExtractionProcessor extends BaseProcessor<BookmarkSourceClassifiedEvent> {
  constructor(
    private readonly firecrawlService: FirecrawlService,
    private readonly webContentRepo: WebContentRepository
  ) {
    super("Content Extraction Processor");
  }
  ...
  protected async processEvent(event: BookmarkSourceClassifiedEvent): Promise<void> {
    ...
    if (!TEXTUAL_SOURCES.includes(source as BookmarkSource)) {
      this.logStep("Skipping content extraction for non-textual source", {
        bookmarkId,
        source,
      });
      return;
    ...
    try {
      this.logStep("Starting content extraction", {
        bookmarkId,
        url,
        source,
      });
      ...
      const existing = await this.webContentRepo.findByBookmarkIdInternal(bookmarkId);

      if (existing && existing.status !== ContentStatus.PENDING) {
        this.logStep("Content already processed or in progress, skipping", {
          bookmarkId,
      ...
      if (!existing) {
        ...
        this.logStep("Created pending web content record", { bookmarkId });
      }

      // Mark as processing
      await this.webContentRepo.markAsProcessing(bookmarkId);
      ...
      this.logStep("Content extraction metrics calculated", {
        bookmarkId,
        wordCount,
        charCount,
      ...
      this.logStep("Content extraction completed successfully", {
        bookmarkId,
        wordCount,
      ...
      this.logStep("Published content-extracted event", {
        bookmarkId,
        messageId,
      });
...
const contentExtractionProcessor = new ContentExtractionProcessor(
  firecrawlService,
  webContentRepo
);
...
export async function handleContentExtraction(
  ...
  return contentExtractionProcessor.safeProcess(event);
}
...
export const contentExtractionSubscription = new Subscription(
  ...
  "content-extraction-processor",
  {
    handler: (event) => contentExtractionProcessor.safeProcess(event),
  }
);
